(ns arm_isa.main
  "Kotodama WASM entrypoint for the Arm_isa clean-room actor (L4) — Clojure port.
  ...
  facts (`arm_isa.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "arm_isa")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"arm_isa.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "arm_isa-compat" :tier tier :entities entities} 200])
...
(defn start! [] :arm_isa-compat/ready)
